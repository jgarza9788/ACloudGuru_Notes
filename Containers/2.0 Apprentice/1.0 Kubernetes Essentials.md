...
Here's a summary of the key points about Kubernetes services:

* Services in Kubernetes allow dynamic access to groups of replica pods.
* Services provide load balancing for accessing pods, even when the pod list is dynamic and constantly changing.
* A service acts as an abstraction layer on top of a set of replica pods with the same label selectors.
* The selector defines which pods the service routes traffic to based on their labels.
* There are different types of services in Kubernetes, and one type is NodePort, which allows external access to the service.
* NodePort services listen on a specific port on all cluster nodes and route traffic to pods.
* Kubernetes also supports ClusterIP services for internal communication within the cluster.
* Services are useful for load balancing, providing stable access to pods, and facilitating communication among pods.
* Services abstract the complexity of routing traffic to specific pods, making it easier to work with replica pods.
* Creating a service involves specifying label selectors and defining ports for routing traffic.
* Services can be accessed from within the cluster or externally, depending on their type.

These key points provide an overview of Kubernetes services and their role in facilitating communication and load balancing within a cluster.
...
Here's a summary of the key points about microservices and their benefits:

* Microservices are an application architecture where an application is built around small, independent services that work together.
* This contrasts with the traditional monolithic approach where all application features are combined into a single executable.
* Microservices break the application into separate executables that can be * independently scaled, written in different languages, and use different technologies.
* Advantages of microservices include scalability, cleaner code, and reliabi* lity, as changes in one area are less likely to affect others.
* Microservices allow for a wider variety of tools and technologies to be used, tailoring each part of the application to its specific requirements.
* While microservices offer many benefits, they also introduce complexity in managing and deploying individual components.
* Kubernetes is well-suited for managing and automating the complexities of microservices, making it an ideal platform for microservice-based applications.

Microservices provide flexibility and scalability in application design but require robust management, which Kubernetes can efficiently handle. In the next lesson, the deployment of a microservice application in Kubernetes will be explored in more detail.


**Decomposition:**
The application's functionality is decomposed into multiple individual services, with each service responsible for a specific business capability or function.

**Independence:**
Microservices are designed to be independent of each other. They can be developed, deployed, and scaled independently without affecting the entire application.

**Loose Coupling:**
Services communicate with each other through well-defined APIs (often using lightweight protocols like HTTP or gRPC). They are loosely coupled, meaning that changes to one service should not disrupt others.

**Autonomy:**
Each microservice can be developed by a separate team, allowing for parallel development and faster innovation.

**Scalability:**
Services can be scaled independently based on their specific resource requirements and workload demands. This enables efficient resource utilization.

**Resilience:**
Failures in one service should not cascade to affect the entire application. Microservices are designed to be fault-tolerant and can often handle failures gracefully.

**Technology Diversity:**
Different services can use different programming languages, frameworks, and databases, choosing the most appropriate technology stack for their specific tasks.

**Deployment and DevOps:**
Microservices typically involve automated deployment processes and DevOps practices to ensure continuous integration, delivery, and monitoring.


# Deploying the Robot Shop App

* Stan's Robot Shop, a sample microservices app, is deployed to the Kubernetes cluster for demonstration purposes.

* The Robot Shop application is composed of various microservices, each responsible for a specific feature or function within the application.

* A custom namespace, "robot-shop," is created in the Kubernetes cluster to isolate all components related to the Robot Shop app.

* YAML descriptors are provided for different Kubernetes objects, such as deployments and services, to define the components of the application.

* The kubectl command is used to apply these descriptors to the cluster, creating pods and services for each microservice.

* Pods representing various microservices are observed as they transition to the "Running" state.

* The Robot Shop application is accessed via a web browser, showcasing its various features, which are implemented as separate microservices.

* Microservices in the application are designed to use different technologies and databases, highlighting the flexibility of microservices architecture.

Kubernetes simplifies the deployment, scaling, and management of complex microservices applications by providing orchestration and automation capabilities.

* Kubernetes allows for efficient scaling of individual microservices and simplifies communication between microservices.

* The Robot Shop application serves as an example of how Kubernetes can effectively manage microservices, making the deployment and maintenance of complex applications more manageable.

Kubernetes handles many of the complexities associated with deploying and managing microservices, allowing developers to focus on application functionality rather than infrastructure management.

* The demonstration provides an overview of microservices within Kubernetes, emphasizing the benefits of this architecture for building scalable and maintainable applications.
...
# create a namespace, and create the descriptors
# these are just yml files


kubectl get pods -n robot-shop -w
# list the pods for the robot-shop
